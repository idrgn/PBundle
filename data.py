...
def p3hash(data, mode):
    with tempfile.NamedTemporaryFile(mode='w+b', suffix=".bin", delete=False) as tf:
        temp = tf.name
        tf.write(data)
        check_output(resource_path("res\ext\p3h.exe") + " " + tf.name + " " + tf.name + "1 " + mode, shell=True)
...
def read_integer(fdata, position, size):
    return int.from_bytes(read_byte_array(fdata, position, size), byteorder='little', signed=False)

def read_byte_array(fdata, position, size):
    if position + size > len(fdata): size = len(fdata) - position
